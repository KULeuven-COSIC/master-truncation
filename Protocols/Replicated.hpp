        ...
        false_type)
{
    ...
    octetStream cs;
    std::vector<value_type> masked_s_all;
    ...
    for (auto info : infos)
        ...
        {
            auto& x = S[info.source_base + i];
            // std::cout << "share0: " << x[0] << " share1: " << x[1] << std::endl;
            value_type masked_s = x[0] - r_prime;
            masked_s_all.push_back(masked_s);
            // std::cout << "masked s i: " << masked_s_all[i] << std::endl;
            masked_s.pack(cs);
        }
    // std::cout << "P" << proc.P.my_num() << ": Sending ";
    // print_os(cs);
    ...
    // std::cout << "P" << proc.P.my_num() << ": received from P" << positive_modulo(proc.P.my_num()+1, 3) << " ";
    // print_os(cs);
    // std::cout << std::endl;
    P.receive_relative(-1, os);
    // std::cout << "P" << proc.P.my_num() << ": received from P" << positive_modulo(proc.P.my_num()-1, 3) << " ";
    ...
    for (auto info : infos)
        for (int i = 0; i < size; i++)
        {
            ...
            // std::cout << "result0: " << y[0] << " result1: " << y[1] << std::endl;
        }
    
#elif defined(ABY3_SEMI_TRUNC)
    assert(regs.size() % 4 == 0);
    assert(proc.P.num_players() == 3);
    assert(proc.Proc != 0);
    typedef typename T::clear value_type;
    ...
    if (generate)
    {
        ...
        for (auto info : infos)
            for (int i = 0; i < size; i++)
            {
                ...
                y[1].pack(cs);
                y[0] = r;
                // std::cout << "P" << proc.P.my_num() << " y0 = " << y[0] << ", y1 = " << y[1] << std::endl;
            }
        // std::cout << "P" << proc.P.my_num() << ": Sending ";
        // print_os(cs);
        // std::cout << " to P " << comp_player << std::endl;
    ...
    if (compute)
    {
        ...
        // std::cout << "P" << proc.P.my_num() << ": Received ";
        // print_os(cs);
        // std::cout << " from P " << gen_player << std::endl;
        for (auto info : infos)
            for (int i = 0; i < size; i++)
            {
                auto& x = S[info.source_base + i];
    ...
    if ( not (compute or generate))
    {
        ...
        for (auto info : infos) {
          // std::cout << "P" << proc.P.my_num() << " big_gap = "<< info.big_gap() << std::endl;
            if (info.big_gap())
                for (int i = 0; i < size; i++)
                {
                    auto& x = S[info.source_base + i];
